...
|Used by serializers and deserializers when `BACKGROUND_REFRESH_ENABLED` is enabled. Specifies the maximum time (milliseconds) to wait for a background refresh operation to complete before timing out. If a background refresh exceeds this timeout, it will be interrupted and the stale value will continue to be served until the next refresh attempt.
|`java.time.Duration, non-negative Number, or integer String`
|`30000`
|`USE_ID`
...
Starting with {registry} version 3.x, the Protobuf SerDes implementation uses *protobuf4j*, which runs the official Google `protoc` compiler via WebAssembly (WASM). This provides 100% compatibility with native protobuf behavior and improved schema canonicalization.

Key benefits of the protobuf4j implementation:

* *100% protoc compatibility*: Uses the actual `protoc` binary compiled to WebAssembly
* *Improved canonicalization*: Guaranteed byte-for-byte consistency for semantically equivalent schemas
* *Reduced dependencies*: Significantly smaller dependency footprint compared to previous implementations

NOTE: If you are upgrading from an earlier 3.x version that used Wire-Schema, the new protobuf4j implementation maintains full backward compatibility. Old clients work with new servers and vice versa. No code changes are required for standard usage patterns.
