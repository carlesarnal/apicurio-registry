...
.Rule execution REST API endpoint
[cols="1,4,3", options="header"]
|===
|Method |Endpoint |Description
...
Uses real CEL (Common Expression Language) via the `cel-standalone` library. Full CEL spec support including parenthesized grouping, ternary expressions, string functions, `has()`, `size()`.
