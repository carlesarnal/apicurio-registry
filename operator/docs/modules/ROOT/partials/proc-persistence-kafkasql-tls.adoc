...
The `tls` section uses `SecretKeyRef` references that point to the secrets created by {kafka-streams}:
+
[source,yaml]
----
...
----
+
Each `SecretKeyRef` field has a `name` (the secret name) and an optional `key` (the key within the secret).
If the `key` is omitted, the operator uses the following default keys:
+
* `keystoreSecretRef` - defaults to `user.p12`
* `keystorePasswordSecretRef` - defaults to `user.password`
* `truststoreSecretRef` - defaults to `ca.p12`
* `truststorePasswordSecretRef` - defaults to `ca.password`
+
These defaults match the key names produced by the {kafka-streams} Operator, so you typically only need to specify the secret `name`.
...
TIP: As an alternative to manual TLS configuration, you can use the Strimzi Kafka Access Operator for automatic setup (available starting with {registry} version 3.2.0). See xref:registry-persistence-kafkasql-access[].
